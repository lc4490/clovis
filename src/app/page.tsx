  ...
  return (
    <div className="flex h-[100dvh] overflow-hidden font-sans text-clover-dark bg-clover-bg" style={{ paddingTop: "env(safe-area-inset-top)" }}>
      ...
      {sidebarOpen && (
        <div className="sm:hidden fixed inset-0 z-40">
          ...
          <div className="absolute inset-y-0 left-0 z-50">
            <Sidebar onQuickAction={handleSidebarAction} member={member} language={language} onClose={() => setSidebarOpen(false)} fontSize={TEXT_SIZES[textIdx]} />
          </div>
        </div>
      )}
      ...
      <div className="hidden sm:flex h-full">
        <Sidebar onQuickAction={handleSidebarAction} member={member} language={language} fontSize={TEXT_SIZES[textIdx]} />
      </div>
